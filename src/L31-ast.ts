...
const parseClassExp = (fields:Sexp, methods:Sexp): Result<ClassExp>=>
     {
        ...
        if( !(isArray(fields) && allT(isString, fields) && fields.length>=1))
        ...
        const bindingsResult = bind(valsResult, (vals: CExp[]) => makeOk(zipWith(makeBinding, vars, vals)));
        return bind(bindingsResult, (bindings: Binding[])=> makeOk(makeClassExp(map(makeVarDecl, fields), bindings)));
...
const unparseClssExp = (ce: ClassExp):string=>
...
export const unparseL31 = (exp: Program | Exp): string =>
    ...
    isClassExp(exp)? unparseClssExp(exp):
    exp;


const myIfExp = bind(parseSexp("(define b 1)\n(define c 2)\n(define pair\n(class (a b)\n((first (lambda () a))\n(second (lambda () b))\n(f (lambda () (+ a b c)))))"), parseL31Exp);
console.log(JSON.stringify(myIfExp, null))